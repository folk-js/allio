...
pub fn get_cf_dictionary_value<T>(dict: &CFDictionary, key: &str) -> Option<*const T> {
...
pub fn get_cf_number(dict: &CFDictionary, key: &str) -> i32 {
  unsafe {
    let mut value: i32 = 0;
...
pub fn get_cf_boolean(dict: &CFDictionary, key: &str) -> bool {
  unsafe {
...
pub fn get_cf_string(dict: &CFDictionary, key: &str) -> String {
  unsafe {
...
pub fn get_cf_window_bounds(dict: &CFDictionary) -> Option<CGRect> {
  match get_cf_dictionary_value::<CFDictionary>(dict, "kCGWindowBounds") {
    Some(dict_rect) => unsafe {
      ...
      if !dict_rect.is_null()
        && CGRectMakeWithDictionaryRepresentation(Some(&*dict_rect), &mut cg_rect)
      {
        Some(cg_rect)
      } else {
        None
...
pub fn retain_cf_dictionary(ptr: *const CFDictionary) -> Option<CFRetained<CFDictionary>> {
  if ptr.is_null() {
    None
  } else {
