...
use objc2_core_foundation::{
    CFArray, CFBoolean, CFDictionary, CFNumber, CFNumberType, CFRetained, CFString, CGPoint,
    CGRect, CGSize,
};
...
fn get_windows_informations(only_active: bool) -> Result<Vec<WindowInfo>> {
    ...
    // Get only currently onscreen windows - no state tracking
    let onscreen_options =
        CGWindowListOption::OptionOnScreenOnly | CGWindowListOption::ExcludeDesktopElements;
    let window_list_info: &CFArray = { &CGWindowListCopyWindowInfo(onscreen_options, 0).unwrap() };
    ...
    match app {
        Ok(app) => {
            ...
            let mut command = format!(
                "tell app id \"{bundle_identifier}\" to get URL of active tab of front window"
            );
            if is_from_document(&bundle_identifier) {
                command =
                    format!("tell app id \"{bundle_identifier}\" to get URL of front document");
            }
